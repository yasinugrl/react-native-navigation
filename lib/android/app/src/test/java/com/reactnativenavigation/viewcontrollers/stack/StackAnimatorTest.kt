...
class StackAnimatorTest : BaseTest() {
    ...
    override fun beforeEach() {
        ...
        uut = StackAnimator(activity, transitionAnimatorCreator)
    ...
    fun push_exitAndEnter() {
        ...
        val exit = pushAnimator.childAnimations[1] as AnimatorSet
    ...
    fun pop_onlyExitAnimationIsPlayedByDefault() {
        ...
        val popAnimator = uut.runningPopAnimations[child2]!!
        assertThat(popAnimator.childAnimations).hasSize(1)

        val disappearingAnimatorSet = popAnimator.childAnimations.first() as AnimatorSet
    ...
    fun pop_enterExitAnimations() {
        ...
        val popAnimator = uut.runningPopAnimations[child2]!!
        assertThat(popAnimator.childAnimations).hasSize(2)

        val enter = popAnimator.childAnimations[1] as AnimatorSet
        (enter.childAnimations).forEach {
            assertThat((it as ObjectAnimator).target).isEqualTo(child1.view)
        }

        val exit = popAnimator.childAnimations.first() as AnimatorSet
        (exit.childAnimations).forEach {
            assertThat((it as ObjectAnimator).target).isEqualTo(child2.view)
        }
    }
